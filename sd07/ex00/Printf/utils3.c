...
{
	if (flag != 0)
		f->nbr_precision = ft_atoi_printf(&stamp[*i]);
	else
		f->nbr_width = ft_atoi_printf(&stamp[*i]);
...
void	handle_width_str(t_struct *f, int i, int *leng)
{
	...
	if (!ft_strncmp_printf(f->str, "(null)", 6) && f->nbr_precision < 6 && f->dot == 1)
		len = -6;
	else
	{
		len = f->nbr_precision - ft_strlen_printf(f->str);
		if (len < 0 && f->dot == 1)
			len = 0;
	}
	if (f->minus == 0)
	{
		i = 0;
		if (ft_strncmp_printf(f->str, "(null)", 6))
...
void	handle_width_nbr(t_struct *f, int i, int *leng)
{
	...
	if (ft_strlen_printf(f->str) > f->nbr_precision)
		len = f->nbr_width - ft_strlen_printf(f->str)
...
int	ft_strncmp_printf(char *s1, char *s2, unsigned int n)
{
	unsigned int	i;
